...
const ClientsScroller = () => {
  ...
  useEffect(() => {
    ...
    }, 3000); // Slowed down for smoother effect
  ...
  return (
    <div className="relative w-full py-12 bg-white text-center px-4 sm:px-8 md:px-16">
      <h2 className="text-2xl sm:text-4xl md:text-5xl text-[#0a243a] mb-8">
        Our Clients
      </h2>
      ...
      <div className="relative flex items-center justify-center w-full">
        ...
        <button
          ...
          className="absolute left-2 sm:left-4 z-10 p-2 sm:p-3 bg-white rounded-full shadow-md hover:bg-gray-100 focus:outline-none"
        >
          <FaChevronLeft size={20} className="text-blue-700" />
        </button>

        {/* Scrollable Container */}
        <div
          ref={scrollRef}
          className="flex gap-6 sm:gap-8 overflow-hidden scroll-smooth whitespace-nowrap px-4 py-2"
        >
          {clients.map((client, index) => (
            ...
            >
              <img
                src={client.logo}
                alt={client.name}
                className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 object-contain mb-2 transition-transform hover:scale-105"
              />
        ...
        <button
          ...
          className="absolute right-2 sm:right-4 z-10 p-2 sm:p-3 bg-white rounded-full shadow-md hover:bg-gray-100 focus:outline-none"
        >
          <FaChevronRight size={20} className="text-blue-700" />
        </button>
      </div>
    </div>
  );
